...
class inverseISP(nn.Module):
    def __init__(self, freeze_linear=False, freeze_flow=False, use_flow=True, subnet_constructor=subnet('RBNet'),
                 block_num=1, wb=None, ccm=None, global_size=128):
        super().__init__()
        operations = []
        channel_num = 3
        channel_split_num = 1
        ...
        if freeze_flow:
            set_requires_grad(self.operations, False)
        if freeze_linear:
            self.wb.required_grad = False
            self.ccm.required_grad = False
            set_requires_grad(
                [self.ccm_estimator, self.bright_estimator, self.wb_estimator, self.wb_evaluator, self.bright_evaluator,
                 self.ccm_evaluator], False)
    ...
    def forward(self, x, rev=False):
        ...
        if self.training and dist.get_rank() == 0 and not rev:
            if self.counter == 600:
                self.counter = 0
